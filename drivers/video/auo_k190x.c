				...
				struct list_head *pagelist)
{
	...
	if (time_is_after_jiffies(par->last_temperature + HZ*5)) {
...
static int auok190xfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
...
/* This is an epd, so there is no need to blank anything. */
static int auok190xfb_blank(int blank_mode, struct fb_info *info)
{
	return 0;
}

/*static int auok190xfb_setcmap(struct fb_cmap *cmap, struct fb_info *info)
{
//	printk("set cmap\n");
	return 0;
}*/
 ...
 * Can be used to let the driver know the current usecase.
 * Valid types are: 
 * animation .. the display content is changing at a very high rate
 * gui .. default, full "color" spectrum, resonable update times
 * text-image .. text interleaved with images; minimize artifacts
 * text-only .. text only display, minimize artifacts
 * highspeed .. do the fastest updates possible, disregarding display artifacts
 */
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	switch (par->content_type) {
	case GUI:
		return sprintf(buf, "gui\n");
		break;
	case ANIMATION:
		return sprintf(buf, "animation\n");
		break;
	case TEXT_IMAGE:
		return sprintf(buf, "text_image\n");
		break;
	case TEXT_ONLY:
		return sprintf(buf, "text_only\n");
		break;
	case HIGHSPEED:
		return sprintf(buf, "highspeed\n");
		break;
	default:
				  ...
				  const char *buf, size_t count)
{
	...
	/* check for callback existence */
	if (!par->set_content_type)
		return -EINVAL;

	if (!strncmp(buf, "gui", 3))
		par->set_content_type(par, GUI);
	else if (!strncmp(buf, "animation", 9))
		par->set_content_type(par, ANIMATION);
	else if (!strncmp(buf, "text_image", 10))
		par->set_content_type(par, TEXT_IMAGE);
	else if (!strncmp(buf, "text_only", 9))
		par->set_content_type(par, TEXT_ONLY);
	else if (!strncmp(buf, "highspeed", 9))
		par->set_content_type(par, HIGHSPEED);
	else
		return -EINVAL;
...
static DEVICE_ATTR(content_type, 0644, content_type_show, content_type_store);

static struct attribute *auok190x_attributes[] = {
	&dev_attr_update_mode.attr,
	&dev_attr_flash.attr,
	&dev_attr_temp.attr,
	&dev_attr_content_type.attr,
	NULL
};
				    ...
				    struct auok190x_init_data *init)
{
	...
	par->init = init->init;
	par->set_content_type = init->set_content_type;
