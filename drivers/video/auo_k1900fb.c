...
/* 1 param = 0, bound to use_100hz
 * FIXME: more info needed
 */
#define AUOK1900_CMD_LUT_FREQUENCY	0x0004
/* FIXME: probably not present at all */
#define AUOK1900_CMD_FULLDISP		0x1000
...
static void auok1900_set_content_type(struct auok190xfb_par *par,
				      enum auok190x_content_type content_type)
{
	/* do a deep refresh when entering a reading mode */
	if (content_type == TEXT_IMAGE || content_type == TEXT_ONLY) {
		auok1900_update_region(par, AUOK190X_UPDATE_MODE(2), 0, par->info->var.yres);
		par->update_cnt = 0;
	}

	par->content_type = content_type;
}
...
static int __devinit auok1900fb_probe(struct platform_device *pdev)
{
	...
	init.set_content_type = auok1900_set_content_type;
